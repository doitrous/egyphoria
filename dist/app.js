...
function renderTrips(filter='all'){
  ...
  $('#trip-grid').innerHTML=shown.map(t=>`<article class="trip-card"><button class="trip-image" data-trip="${t.id}" aria-label="Explore ${t.name}"><img src="assets/${t.image}.jpg" alt="${t.image==='alexandria'?'Qaitbay Citadel in Alexandria':t.image==='nile'?'Sailboats on the Nile':t.image==='giza'?'Pyramids of Giza at sunset':t.image==='luxor'?'Ancient Egyptian temple columns and statues':t.image==='cairo'?'Lanterns in Khan el-Khalili, Cairo':t.image==='desert'?'Chalk formations in Egypt’s White Desert':'Egypt’s Red Sea coast'}" loading="lazy"><span class="trip-badge">${t.tag}</span><span class="trip-arrow" aria-hidden="true"><svg class="link-icon" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="square" stroke-linejoin="miter" aria-hidden="true" focusable="false"><path d="M14 3h7v7M21 3 10 14M10 3H3v18h18v-7"/></svg></span></button><p class="trip-meta">${t.location}</p><h3><button data-trip="${t.id}">${t.name}</button></h3><p class="trip-description">${t.description}</p><div class="trip-footer"><span>${t.days} ${t.days===1?'day':'days'} · ${t.kind==='multi'?'Multi-day journey':'Day experience'}</span><span>Make it yours</span></div></article>`).join('');
}
